...
SANP does not claim to solve every open problem in the field. It proposes a structure to name them clearly, and explicitly identifies the areas that remain active research rather than solved engineering.
...
AI structurally breaks with this principle. An AI system must **understand content** to function: routing a request to the right model requires interpreting its meaning, not just reading an address. SANP starts from this observation: this is not a *transport* model, it is a model of **distributed cognition**. The analogy with OSI remains pedagogically useful, but it should not be stretched beyond its actual relevance.

### 1.1 Related work

This document does not start from a blank slate, and presenting it as such would be dishonest. Several recent initiatives address closely related problems:

- **Agent Network Protocol (ANP)** — a distinct open-source protocol (unrelated to this document, despite the acronym overlap in its early drafts) aiming to become the "HTTP of the agentic web," with a three-layer architecture (identity/encryption, meta-protocol, application) for peer-to-peer agent interconnection. Important prior art for anyone working in this space.
- **AI-Native Network Protocol (AINP)** — an IETF Internet-Draft (November 2025) that replaces location-based routing with semantic routing and byte-stream exchange with intent exchange — an intuition very close to SANP's Layer 2.
- **MCP (Model Context Protocol)** and **A2A (Agent-to-Agent)** — already referenced in this document, respectively focused on model-tool connection and inter-agent collaboration in trusted environments.
- **SANNet** — a semantic-aware agentic AI networking framework for multi-agent cross-layer coordination, proposed for IEEE GLOBECOM 2025.

SANP does not claim to replace these initiatives, but proposes a different architectural reading: an explicit layered structure including an energy dimension (Layer 0) and intrinsic transversal governance (Layer T), two angles that remain underdeveloped in the work cited above.

---

## 2. General architecture

SANP consists of **six numbered layers (0 to 5)** and **one transversal governance layer** that runs through all of them. This architectural choice — six layers rather than an arbitrarily reduced number — reflects the real complexity of the problem rather than a surface-level simplification.

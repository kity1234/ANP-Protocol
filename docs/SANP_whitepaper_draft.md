...
L'IA rompt structurellement avec ce principe. Un système d'IA doit **comprendre le contenu** pour fonctionner : router une requête vers le bon modèle exige d'interpréter son sens, pas seulement de lire une adresse. SANP part de ce constat : ce n'est pas un modèle de *transport*, c'est un modèle de **cognition distribuée**. L'analogie avec OSI reste utile pédagogiquement, mais elle ne doit pas être forcée au-delà de sa pertinence réelle.

### 1.1 Travaux connexes

Ce document ne part pas d'un terrain vierge, et il serait malhonnête de le présenter comme tel. Plusieurs initiatives récentes adressent des problèmes proches :

- **Agent Network Protocol (ANP)** — un protocole open source distinct (sans lien avec ce document, malgré la proximité d'acronyme dans ses premières versions) visant à devenir le « HTTP du web agentique », avec une architecture à trois couches (identité/chiffrement, méta-protocole, application) pour l'interconnexion pair-à-pair d'agents. Une antériorité importante à connaître pour quiconque travaille dans cet espace.
- **AI-Native Network Protocol (AINP)** — un Internet-Draft soumis à l'IETF (novembre 2025), qui remplace le routage par localisation par un routage sémantique et l'échange de flux d'octets par un échange d'intentions — une intuition très proche de la Couche 2 de SANP.
- **MCP (Model Context Protocol)** et **A2A (Agent-to-Agent)** — déjà cités dans ce document, respectivement centrés sur la connexion modèle-outils et la collaboration inter-agents en environnement de confiance.
- **SANNet** — un framework de réseau IA agentique « semantic-aware » pour la coordination multi-agents inter-couches, proposé pour IEEE GLOBECOM 2025.

SANP ne prétend pas remplacer ces initiatives, mais propose une lecture architecturale différente : une structuration explicite en couches incluant une dimension énergétique (Couche 0) et une gouvernance transversale intrinsèque (Couche T), deux angles peu développés dans les travaux cités ci-dessus.

---

## 2. Architecture générale

SANP compte **six couches numérotées (0 à 5)** et **une couche transversale de gouvernance** qui les traverse toutes. Ce choix architectural — six couches plutôt qu'un nombre arbitrairement réduit — reflète la complexité réelle du problème plutôt qu'une simplification de façade.
...
La faiblesse structurelle commune aux couches 0, 2 et T est la suivante : **un système peut déclarer une propriété (budget, compétence, alignement) sans que cette déclaration soit vérifiable par construction.** C'est le même défi que la sécurité des communications a résolu partiellement avec TLS et les autorités de certification. SANP propose de traiter cette question comme un axe à part entière plutôt que comme un détail d'implémentation, en combinant quatre mécanismes complémentaires — aucun n'étant suffisant isolément :
